...
def xy_wh_r_2_xy_sigma_dyna(xywhr,x,y):
    """Convert oriented bounding box to 2-D Gaussian distribution.
    ...
    # 使用广播机制创建最终的张量  
    matrix = torch.zeros(sin_r.shape[0], 2, 2, dtype=torch.float32, device=device)  
    matrix[:, 0, 0] = x.reshape(-1)  # 设置左上角为a的值  
    matrix[:, 1, 1] = y.reshape(-1)  # 设置右下角为b的值  
    # matrix = torch.tensor([[x, 0], [0, y]], dtype=torch.float32, device=device)
    sigma = R.bmm(S.square()*matrix).bmm(R.permute(0, 2,
                                            1)).reshape(_shape[:-1] + (2, 2))
...
def gwd_loss(pred, target, fun='log1p', tau=1.0, alpha=1.0, normalize=True):
    ...
    _t_det_sqrt = (Sigma_p.det().clamp(1e-7) * Sigma_t.det().clamp(1e-7)).clamp(1e-7).sqrt()
...
def kld_loss(pred, target, fun='log1p', tau=1.0, alpha=1.0, sqrt=True):
    ...
    Sigma_p_det = Sigma_p.det().clamp(1e-7).log()
    Sigma_t_det = Sigma_t.det().clamp(1e-7).log()
    # Sigma_p_det = Sigma_p.det().clamp(1e-7)
    # Sigma_t_det = Sigma_t.det().clamp(1e-7)
    whr_distance = whr_distance + 0.5 * (Sigma_p_det-Sigma_t_det)
    whr_distance = whr_distance - 1
    # whr_distance = whr_distance + 0.5 * (Sigma_p_det/Sigma_t_det-1)

    # a = torch.tensor(500.0, dtype=torch.float, device=whr_distance.device)
    # whr_distance = torch.where(Sigma_p.det()*Sigma_t.det()<0,a,whr_distance)
    distance = (xy_distance / (alpha * alpha) + whr_distance.clamp(1e-7))
    if sqrt:
        distance = distance.clamp(1e-7).sqrt()

    distance = distance.reshape(_shape[:-1]).clamp(1e-7)
    # print(fun,tau)
    return postprocess(distance, fun=fun, tau=tau)

@weighted_loss
def md_loss(pred, target, fun='log1p', tau=1.0, alpha=1.0, sqrt=True):
    """Kullback-Leibler Divergence loss.
...
@weighted_loss
def mix_kld_loss(pred, target, fun='log1p', tau=1.0, alpha=1.0, sqrt=True):
    """Symmetrical Max Kullback-Leibler Divergence loss.
    ...
    # b=gama1+gama2
    # flag= torch.where(a<0,-1,1)
    # x=0.5*(b+(b*b-4*a).clamp(0).sqrt())
    # y=0.5*(b-(b*b-4*a).clamp(0).sqrt())
    # maxwh=torch.where(x>y,x,y)
    # minwh=torch.where(x<=y,x,y)

    # raw_w=torch.where(gama3>=0,maxwh,minwh)
    # raw_h = torch.where(gama3>0,minwh,maxwh)
    # rotate,wh,rotate_t=torch.svd(sigma)
    # raw_w,raw_h = wh[:,0],wh[:,1]
    # w_symb=torch.where(raw_w>=0,1,-1)
    # h_symb=torch.where(raw_h>=0,1,-1)
    # target = xy_wh_r_2_xy_sigma_dyna(target,flag,flag.abs()) 
    kld_pt = kld_loss(
        pred,
        target,
        fun='none',
    ...
    kld_tp = kld_loss(
        ...
        sqrt=sqrt,
        reduction='none')
    kld_symmin = torch.min(kld_pt, kld_tp).nan_to_num(500)
    # pred2<0 and ph>0:choose kld_12
...
def kld_symmin_loss(pred, target, fun='log1p', tau=1.0, alpha=1.0, sqrt=True):
    ...
    Returns:
        loss (torch.Tensor)
    """
    # target = xy_wh_r_2_xy_sigma_dyna(target,1,1)
    kld_pt = kld_loss(
        pred,
        target,
        fun='none',
